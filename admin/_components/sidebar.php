<?php
    ...
    $user = null;
    if ($userId) {
        $stmt = $conn->prepare("SELECT user_name, profile_image FROM users WHERE user_id = ?");
        $stmt->execute([$userId]);
...
<aside id="sidebar" class="!fixed top-0 left-0 h-screen w-68 !bg-white shadow-lg transform -translate-x-full transition-transform duration-300 lg:translate-x-0 lg:relative p-6 !z-[99999]">
    ...
    <nav class="mt-4 space-y-2">
        <a href="<?= $base_url ?>/admin/profile" class="flex items-center px-3.5 py-2.5 text-gray-700 hover:bg-gray-200 rounded-lg sidebar-link-item mb-5 border border-gray-200">
            <span class="flex items-center border-l-2 border-[#1d2a62] pl-[10px]">
                ...
                <span class="flex flex-col gap-[2px]">
                    <span class="ml-3 text-[12px] font-[500]"><?= htmlspecialchars($user['user_name'] ?? 'Admin') ?></span>
                    <span class="ml-3 text-[11px] text-primary">Admin</span>
                </span>
            </span>
        </a>
