...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['email'])) {
    ...
    if (empty($email)) {
        header("Location: " . $base_url . "/admin/forgot-password.php?error=Email is required");
        exit();
    }
    ...
    if ($stmt->rowCount() === 0) {
        header("Location: " . $base_url . "/admin/forgot-password.php?error=Email not found in our records");
        exit();
    }
    ...
    if ($stmt->rowCount() > 0) {
        $reset_link = "http://" . $_SERVER['HTTP_HOST'] . $base_url . "/admin/reset-password.php?token=" . $token;
        ...
        try {
            ...
            $mail->send();
            header("Location: " . $base_url . "/admin/forgot-password.php?status=success");
            exit();
            
        } catch (Exception $e) {
            header("Location: " . $base_url . "/admin/forgot-password.php?error=Error sending email: " . $mail->ErrorInfo);
            exit();
        }
    } else {
        header("Location: " . $base_url . "/admin/forgot-password.php?error=Error creating reset token");
        exit();
    }
}
